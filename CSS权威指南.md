...
文本正常地流向外部元素。然后，外部文本在第 1 行的末尾被流向其正常的流动位置和尺寸。然后，包含文本的内联框（分布在三行）作为一个单位被移位'-12px'（向上）。
...
文本正常流向内框，内框被拉出流向并浮动到右边的空白处（其 "宽度 "已被明确分配）。浮动左边的线框被缩短，而文档中的剩余文本则流向它们。
...
下面的例子显示了一个绝对定位的盒子，它是一个相对定位的盒子的孩子。尽管父级的 outerbox 实际上没有偏移，但将其'position'属性设置为'relative'意味着它的 box 可以作为定位的后代的包含块。由于外框是一个被分割成几行的内联框，第一个内联框的顶部和左侧边缘（在下图中用粗虚线描述）作为 "顶部 "和 "左侧 "偏移的参考。
...
3. 内流的、非内线级别的、非定位的子元素。
4. 非定位的浮子。
5. 流入的、内联级的、非定位的子代，包括内联表和内联块。
6. 堆栈级别为 0 的子堆栈上下文和堆栈级别为 0 的定位后裔。
7. 具有正的堆叠级别的子堆叠上下文（最少的正的先）。

在每个堆叠上下文中，堆叠级别为 0 的定位元素（在第 6 层）、非定位浮子（第 4 层）、内联块（第 5 层）和内联表（第 5 层）被涂抹，就像这些元素本身产生了新的堆叠上下文一样，除了它们的定位后裔和任何可能的子堆叠上下文参与到当前的堆叠上下文。

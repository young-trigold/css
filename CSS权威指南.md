...
- :hover 伪类在用户用指向设备指定一个元素时适用，但不一定会激活它。例如，一个可视化的用户代理可以在光标（鼠标指针）悬停在一个由元素产生的盒子上时应用这个伪类。不支持交互式媒体的用户代理不需要支持这个伪类。一些支持交互式媒体的符合要求的用户代理可能无法支持这个伪类（例如，一个不检测悬停的笔设备）。
...
下面的值只对父级行内元素有意义，或对父级块状容器元素的支柱有意义。

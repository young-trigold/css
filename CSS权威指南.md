...
| E:empty                        | 一个 E 元素，没有子项(包括文本节点)                                          | 结构性伪类                                                                   | 3          |
...
要表示一个 XHTML 主体的所有 h2 子项，除了第一个和最后一个，可以使用下面的选择器：
...
注意：这并没有对获取资源作任何要求，只是要求样式表如何反映在 CSSOM 中并用于像这样的规范中。假设有适当的缓存，一个 UA 只获取一次样式表是完全合适的，即使它被链接或导入多次。
...
一些数字媒体特征，比如说宽度，在布尔上下文中很少有意义，因为它们的值几乎总是大于 0。其他的，比如 `color`，有意义的 0 值：`(color)` 与 `(color>0)` 是相同的，表明设备根本能够显示颜色。
...
虽然这确保了当视口宽度为 320px 时，两套样式不会同时应用，但它没有考虑到由于非整数像素密度(例如在高 DPI 显示器上或由于缩放的结果)而可能出现的 0 星视口尺寸。任何在 320px 和 321px 之间的视口宽度都会导致没有任何样式被应用。
...
例如，这个媒体查询表达了一个样式表在分辨率大于 300 点数/英寸 的设备上使用：
...
彩色媒体特征描述了输出设备的每一个颜色组成的位数。如果设备不是彩色设备，该值为 0。
...
- 高度为'auto'的正常流中的块盒子的底外边距与它的最后一个正常流中的块级子元素的底外边距一起折叠，如果：
  - 盒子没有底部内边距，以及
  - 盒子没有底部边框，以及
  - 子项的底外边距既不会与有间隙的顶外边距一起折叠，也不会(如果盒子的最小高度不为 0)与盒子的顶外边距一起折叠。
...
一个 **行内盒子** 是一个既是行内级的，其内容又参与其包含的行内格式化上下文的盒子。一个 "display" 值为 "inline" 的非替换性元素会产生一个行内盒子。不是行内盒子的行内级盒子(如替换性行内级元素、inline-block 元素和 inline-table 元素)被称为 **原子行内级盒子**，因为它们作为一个单一的不透明盒子参与其行内格式化上下文。
...
- auto
  对于非替换的元素，这个值的效果取决于哪些相关属性的值也是'auto'。详见关于绝对定位的非替换元素的宽度和高度的章节。对于替换性元素，这个值的效果只取决于被替换内容的固有尺寸。详见绝对定位的被替换元素的宽度和高度部分。
...
本规范中对 **绝对定位元素**(或其盒子)的引用意味着该元素的 'position' 属性值为 'absolute' 或 'fixed'。
...
0，否则
只有正常流程中的子代被考虑在内(即浮动框和绝对定位的框被忽略，相对位置的框被考虑，但不考虑其偏移)。请注意，子框可能是一个匿名的块框。
...
绝对定位的子框被忽略，相对位置的子框被认为没有其偏移。请注意，子框可能是一个匿名的块框。
...
用户代理必须通过其相关的基线将非替换的行内框中的字形相互对齐。然后，为每个字形确定 A 和 D。请注意，单个元素中的字形可能来自不同的字体，因此不一定都有相同的 A 和 D。如果行内框根本不包含字形，它将被视为包含一个具有该元素第一个可用字体的 A 和 D 的支柱(宽度为 0 的不可见字形)。
...
在一个内容由行内级元素组成的块状容器元素上，'line-height'指定了该元素中行框的最小高度。这个最小高度由基线以上的最小高度和基线以下的最小深度组成，完全就像每个行框从一个 0 宽度的行内框开始，具有该元素的字体和行高属性。我们把这个假想的盒子称为 "支杆"。(这个名字是受 TeX 的启发)。
...
由于容器的宽度为 300px，只有三个项目适合在一行中出现。它们占据了 240px，剩下 60px 的剩余空间。因为 flex-flow 属性指定了一个多行的弹性容器（由于其值中出现了 wrap 关键字），flex 容器将创建一个额外的行来容纳最后一个项目。
...
- <'flex-shrink'>
  ...
  注意：在分配负空间时，柔性收缩系数要乘以 flex-basis 尺寸。这与项目能够缩小的程度成比例地分配负空间，因此，例如，一个小弹性子项不会在一个较大的弹性子项被明显缩小之前就缩小到零。
- <'flex-basis'>
  这个组件设置了 flex-basis，它指定了弹性基：在自由空间根据 flex 因子分配之前，弹性子项目的初始主要尺寸。
...
一个无单位的零，如果前面没有两个弹性系数，必须被解释为一个弹性系数。为了避免误解或无效的声明，作者必须指定一个带有单位的零<'flex-basis'>组件，或在其前面加上两个 flex 系数。
...
flex: initial
相当于 flex: 0 1 auto。(这是初始值。) 根据宽度/高度属性确定项目的大小。(如果项目的主尺寸属性计算为 auto，这将根据其内容来确定 flex 项目的尺寸)。当有正数的自由空间时，使柔性项目不具有灵活性，但当空间不足时，允许它收缩到最小尺寸。对齐能力或自动页边距可用于沿主轴对齐柔性项目。
flex: auto
...
弹性：无
相当于 flex: 0 0 auto。这个值根据宽度/高度属性来确定项目的大小，但使 flex 项目完全没有灵活性。这与 initial 类似，只是不允许 flex 项目收缩，即使在溢出的情况下。
flex: <正数
相当于 flex: <正数> 1 0。使得 flex 项目变得灵活，并将 flex 基础设置为 0，从而使项目获得 flex 容器中指定比例的自由空间。如果弹性容器中的所有项目都使用这种模式，它们的大小将与指定的弹性系数成比例。
默认情况下，柔性项目不会缩减到低于其最小内容尺寸（最长的单词或固定尺寸元素的长度）。要改变这一点，请设置 min-width 或 min-height 属性。(参见第 4.5 节 "Flex 项目的自动最小尺寸"）。
...
我们鼓励作者使用 flex shorthand 来控制灵活性，而不是直接使用 flex-grow，因为 shorthand 可以正确地重置任何未指定的组件以适应常见的使用。
...
我们鼓励作者使用 flex shorthand 来控制灵活性，而不是直接使用 flex-basis，因为 shorthand 可以正确地重置任何未指定的组件以适应常见的使用。

flex-base 属性设置了 flex 的基础。它接受与宽度和高度属性相同的值，外加内容。

对于除 auto 和 content（以上定义）以外的所有值，flex-basis 的解析方式与水平书写模式中的 width 相同[CSS21]，只是如果一个值在宽度上会解析为 auto，而在 flex-basis 上会解析为 content。例如，flex-base 的百分比值是针对 flex 项目的包含块（即它的 flex 容器）进行解析的；如果该包含块的大小是不确定的，那么 flex-base 的使用值就是 content。作为另一个推论，除非另有规定，例如通过 box-sizing [CSS3UI]，否则 flex-base 决定了内容框的大小。

## 9.7. 对齐

在一个柔性容器的内容完成了它们的弯曲，并且所有柔性项目的尺寸被确定后，它们就可以在柔性容器中被对齐。

边距属性可用于对齐项目，其方式类似于块状布局中的边距，但比它更强大。Flex 项目也尊重 CSS 箱体对齐的属性，它允许在主轴和横轴上对项目进行简单的基于关键字的对齐。这些属性使许多常见的对齐方式变得简单，包括一些在 CSS 2.1 中非常困难的事情，如水平和垂直居中。

注意：虽然这些对齐属性在 CSS Box Alignment [CSS-ALIGN-3]中被定义，但 Flexible Box Layout 在这里复制了相关的定义，以避免产生规范性依赖，从而延缓规范的进展。这些属性只适用于柔性布局，直到 CSS Box Alignment Level 3 完成并定义了它们对其他布局模式的影响。此外，在盒式对齐模块中定义的任何新值将适用于柔性盒式布局；换句话说，盒式对齐模块一旦完成，将取代这里的定义。

### 9.7.1. 自动外边距对齐

本节为非规范性章节。关于页边距如何影响柔性项目的规范性定义，见柔性布局算法部分。

挠性项目上的自动边距与块流程中的自动边距有非常相似的效果。

在计算弹性基数和弹性长度时，自动边距被视为 0。

在通过 justify-content 和 align-self 进行对齐之前，任何正的自由空间都被分配到该维度的自动边距中。

溢出的方框会忽略其自动边距，并向末端方向溢出。

注意：如果自由空间被分配给了自动页边距，那么对齐属性在该维度上将没有任何作用，因为页边距将窃取弯曲后剩余的所有自由空间。
...
justify-content 属性沿柔性容器的当前行的主轴对齐柔性项目。这是在解决了任何灵活长度和任何自动边距之后进行的。通常情况下，当一行中的所有柔性项目都是不灵活的，或者是灵活但已达到其最大尺寸时，它有助于分配剩余的自由空间。当项目溢出行时，它也会对项目的排列进行一些控制。

弹性开始
弹性项目被打包到行的开始位置。行中第一个柔性项目的主起始边缘与行的主起始边缘齐平，随后的每个柔性项目都与前一个项目齐平。
弯曲末端
弹性项目被打包到行的末端。最后一个柔性项目的主端边缘与该行的主端边缘齐平，每个前面的柔性项目与后面的项目齐平。
中心
挠性项目被打包到行的中心。行中的柔性项目彼此齐平，并在行的中心位置对齐，在行的主起始边缘和行的第一个项目之间以及行的主端边缘和行的最后一个项目之间有相等的空间。(如果剩余的自由空间为负数，则柔性项目将在两个方向上平均溢出)。
空间-中间
弹性项目在行中均匀分布。如果剩余的自由空间为负数，或者行中只有一个柔性项目，该值与 flex-start 相同。否则，行中第一个柔性项目的主起始边缘与行的主起始边缘齐平，行中最后一个柔性项目的主端边缘与行的主端边缘齐平，行中其余的柔性项目的分布使任何两个相邻项目之间的间距相同。
围绕空间
弹性项目在行中均匀分布，两端有半尺寸的空间。如果剩余的自由空间为负数，或者行中只有一个柔性项目，该值与 center 相同。否则，行中的柔性项分布为：行中任何两个相邻的柔性项之间的间距相同，第一个/最后一个柔性项与柔性容器边缘之间的间距为柔性项之间间距的一半大小。
...
align-content 属性在横轴有额外空间时，在柔性容器内对齐柔性容器的行，类似于 justify-content 在主轴内对齐单个项目的方式。注意，这个属性对单行的柔性容器没有影响。值有以下含义。

flex-start
行被打包到柔性容器的开始处。Flex 容器中第一行的交叉起始边缘与 Flex 容器的交叉起始边缘齐平，随后的每一行都与前一行齐平。
柔性末端
线条被打包到柔性容器的末端。最后一行的交叉端边缘与柔性容器的交叉端边缘齐平，每个前面的行都与后面的行齐平。
中心
线条被打包到柔性容器的中心。软体容器中的线条相互齐平，并在软体容器中心对齐，在软体容器的交叉起始内容边缘和软体容器中的第一行之间，以及在软体容器的交叉末端内容边缘和软体容器中的最后一行之间有等量的空间。(如果剩余的自由空间为负数，这些行将在两个方向上平均溢出。)
空间-中间
行数均匀地分布在柔性容器中。如果剩余的自由空间为负数，或者在 flex 容器中只有一条 flex 线，这个值与 flex-start 相同。否则，柔性容器中第一行的交叉起始边缘与柔性容器的交叉起始内容边缘齐平，柔性容器中最后一行的交叉末端边缘与柔性容器的交叉末端内容边缘齐平，柔性容器中其余的行被分布，以便任何相邻两行之间的间距相同。
围绕空间
行在柔性容器中均匀分布，两端有一半大小的空间。如果剩余的自由空间为负数，这个值与 center 相同。否则，柔性容器中的线条分布，使得任何相邻两行之间的间距相同，第一/最后一行与柔性容器边缘之间的间距是柔性线条之间间距的一半大小。
伸展
线条拉伸以占据剩余的空间。如果剩余的自由空间为负数，该值与 flex-start 相同。否则，自由空间被平均分配给所有的线，增加它们的交叉尺寸。
注意：只有多行柔性容器在十字轴上有自由空间供线条对齐，因为在单行柔性容器中，唯一的线条会自动拉伸以填补空间。

### 9.7.5. 弹性容器基线

为了让柔性容器本身参与基线对齐（例如，当柔性容器本身是外部柔性容器中的一个柔性项目时），它需要提交最能代表其内容的基线的位置。为此，柔性容器的基线被确定如下（在用顺序重新排序后，并考虑到柔性方向）。

第一/最后一条主轴基线设置
当柔性容器的内联轴与它的主轴相匹配时，它的基线就按如下方式确定。
如果柔性容器最开始/最末端的柔性线上的任何柔性项参与了基线对齐，那么柔性容器的第一/最后一条主轴基线集就从这些柔性项的共享对齐基线中产生。

否则，如果柔性容器至少有一个柔性项，则柔性容器的第一/最后一条主轴基线集从最开始/最后一条柔性项的对齐基线生成。(如果该项目没有与柔性容器的主轴平行的对齐基线，那么首先从其边界边缘合成一个基线)。

否则，该柔性容器没有设置第一/最后一条主轴基线，如果需要，将根据其对齐方式的规则合成一条基线。

第一个/最后一个交叉轴基线设置
当柔性容器的内联轴与它的交叉轴相匹配时，它的基线将被确定如下。
如果该柔性容器至少有一个柔性项，则该柔性容器的第一/最后一条交叉轴基线集是由最开始/最后一个柔性项的对齐基线生成。(如果该项目没有平行于柔性容器横轴的对齐基线，那么首先从其边框边缘合成一个。)

否则，该柔性容器没有设置第一个/最后一个横轴基线，如果需要，将根据其对齐方式的规则合成一个。

当根据上述规则计算基线时，如果贡献基线的盒子有一个允许滚动的溢出值，那么为了确定其基线，该盒子必须被视为处于其初始滚动位置。

在确定表格单元的基线时，柔性容器会像线框或表格行一样提供一个基线。[CSS21]

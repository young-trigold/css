...
一个柔性项目为其内容建立了一个独立的格式化上下文。然而，柔性项目本身是柔性级别的框，而不是块级别的框：它们参与其容器的柔性格式化上下文，而不是块格式化上下文。

注意：阅读本规范的作者可能想跳过下面的盒子生成和静态位置细节。

Flex项目的显示值是块化的：如果生成flex容器的元素的内流子的指定显示值是一个内联级的值，它就会被计算成块级的等值。(参见CSS2.1§9.7[CSS21]和CSS显示[CSS3-DISPLAY]中关于这种类型的显示值转换的细节）。

注意：某些显示值通常会触发在原始盒子周围创建匿名盒子。如果这样的盒子是一个柔性项目，它首先被封锁，所以匿名盒子的创建将不会发生。例如，两个连续的显示为table-cell的flex项目将成为两个独立的显示为block的flex项目，而不是被包裹成一个匿名表格。

在display: table的柔性项目中，表格包装盒会成为柔性项目，并且顺序和align-self属性适用于它。任何标题框的内容都有助于计算表格包装盒的最小内容和最大内容尺寸。然而，就像宽度和高度一样，灵活的长方形也适用于表格框，具体如下：灵活项目的最终尺寸是通过执行布局来计算的，就像表格包装框的边缘和表格框的内容边缘之间的距离都是表格框的边界+填充区域的一部分，而表格框就是灵活项目。

### 9.3.1. 绝对定位的弹性子项

由于它是流外的，一个绝对位置的柔性容器的子节点不参与柔性布局。

一个绝对位置的柔性容器的子节点的静态位置是这样确定的：假设子节点和柔性容器都是固定尺寸的盒子，那么子节点的位置就如同它是柔性容器中唯一的柔性项目。为了这个目的，自动边距被视为零。

换句话说，一个绝对位置的柔性容器的子项的静态位置矩形是柔性容器的内容框，其中静态位置矩形是用于确定绝对位置的框的静态位置偏移的对齐容器。
(在块状布局中，静态位置矩形对应于CSS2.1§10.3.7中描述的 "假设框 "的位置。因为它没有对齐属性，所以CSS2.1总是在静态位置矩形中使用绝对位置的盒子的块开始内行开始对齐。请注意，这个定义最终会转移到CSS定位模块中）。

这样做的效果是，如果你在一个绝对位置的柔性容器的子节点上设置，例如，align-self: center;，子节点上的自动偏移将使其在柔性容器的横轴上居中。
然而，由于绝对位置的盒子被认为是 "固定尺寸 "的，拉伸的值被视为与flex-start相同。

### 9.3.2. 弹性子项的内外边距

相邻柔性项目的边距不会折叠。

弹性项目上的百分比边距和间距，就像方框上的边距和间距一样，是根据其包含的块的内联尺寸来确定的，例如，在水平书写模式下，左/右/上/下的百分比都是根据其包含块的宽度来确定的。

自动页边距在相应的维度上扩展以吸收额外的空间。它们可用于对齐，或将相邻的柔性项目分开。请参阅使用自动边距进行对齐。

### 9.3.3. 弹性子项 Z-轴顺序

灵活性项目与内联块[CSS21]完全相同，只是使用了修改过的文档顺序来代替原始文档顺序，并且除了自动以外的z-index值会创建一个堆叠上下文，即使位置是静态的（行为与相对位置完全一样）。

注意：定位在flex item之外的子代仍然参与flex item建立的任何堆叠上下文。

### 9.3.4. 折叠子项

在柔性项目上指定visibility:collapse会使其成为一个折叠的柔性项目，产生类似于table-row或table-column的visibility:collapse的效果：折叠的柔性项目被完全从渲染中移除，但会留下一个 "支柱 "以保持柔性行的交叉尺寸稳定。因此，如果一个柔性容器只有一条柔性线，动态折叠或解除折叠的项目可能会改变柔性容器的主尺寸，但保证对其交叉尺寸没有影响，也不会导致页面布局的其他部分 "摇摆"。然而，在折叠后会重新进行柔性线包装，因此具有多条线的柔性容器的交叉尺寸可能会也可能不会改变。

尽管折叠的柔性项目不会被渲染，但它们确实出现在格式化结构中。因此，与display:none项[CSS21]不同的是，依赖于方框出现在格式化结构中的效果（如递增计数器或运行动画和过渡）仍然在折叠的项上运行。

为了计算支杆的大小，首先在所有项目都不折叠的情况下进行柔性布局，然后在每个折叠的项目被一个支杆取代的情况下重新运行，该支杆保持了该项目原来的行的交叉尺寸。关于visibility:collapse如何与flex layout交互的规范性定义，请参见Flex Layout Algorithm。

注意：在任何柔性项目上使用visibility:collapse将导致柔性布局算法在中途重复，重新运行最昂贵的步骤。建议作者继续使用display:none来隐藏项目，如果这些项目不会被动态折叠和取消折叠，因为这对布局引擎来说更有效率。(因为当可见性改变时，只有部分步骤需要重复，然而，对于动态情况，仍然推荐使用 "可见性：折叠")。

### 9.3.5. 弹性子项的自动最小大小

注意：代表自动最小尺寸的auto关键字，是min-width和min-height属性的新初始值。该关键字以前在本规范中定义，但现在在CSS尺寸模块中定义。

为了给柔性项目提供一个更合理的默认最小尺寸，在不是滚动容器的柔性项目上，主轴自动最小尺寸的使用值是基于内容的最小尺寸；对于滚动容器，自动最小尺寸照例是0。

一般来说，柔性项目的基于内容的最小尺寸是其内容尺寸建议和其指定尺寸建议中较小的一个。然而，如果盒子有一个长宽比并且没有指定的尺寸，其基于内容的最小尺寸是其内容尺寸建议和其转移尺寸建议中的较小者。如果盒子既没有指定尺寸建议，也没有长宽比，其基于内容的最小尺寸就是内容尺寸建议。
...
为了计算盒子的内在尺寸（例如盒子的最小内容尺寸），基于内容的最小尺寸导致盒子在该轴上的尺寸变得不确定（即使例如其宽度属性指定了一个确定的尺寸）。请注意，这意味着针对该尺寸计算的百分比将表现为自动。

尽管如此，尽管在某些情况下可能需要额外的布局传递来重新解决百分比问题，但这个值（就像[CSS-SIZEING-3]中定义的min-content、max-content和fit-content值一样）并不妨碍解决项目中的百分比尺寸。

请注意，虽然基于内容的最小尺寸通常是合适的，并有助于防止内容重叠或溢出其容器之外，但在某些情况下它并不合适。
特别是，如果一个文档的主要内容区域使用了弹性尺寸，那么最好设置一个明确的字体相关的最小宽度，如min-width: 12em。基于内容的最小宽度可能会导致一个大的表格或大的图像将整个内容区的尺寸拉伸到一个溢出区，从而使文本行无端地变长，难以阅读。

还要注意的是，当基于内容的尺寸被用在一个有大量内容的项目上时，布局引擎必须在找到其最小尺寸之前遍历所有这些内容，而如果作者设置了一个明确的最小尺寸，这就没有必要。(然而，对于内容较少的项目，这种遍历是微不足道的，因此不存在性能问题）。

## 9.4. 顺序和方向

弹性容器的内容可以以任何方向和任何顺序排列。这使得作者可以简单地实现以前需要复杂或脆弱的方法的效果，如使用浮动和清除属性的黑客。这个功能是通过flex-direction、flex-wrap和order属性来实现的。

注意：Flex布局的重新排序功能有意地只影响视觉渲染，而让语音顺序和导航基于源顺序。这允许作者在操纵视觉呈现的同时，为非CSS UA和线性模型（如语音和顺序导航）保留源顺序。关于使用这种二分法来提高可及性的例子，请参见重新排序和可及性以及Flex布局概述。

作者不得使用顺序或flex-flow/flex-direction的*-reverse值来替代正确的源排序，因为这样会破坏文档的可访问性。
...
Flex-direction 属性通过设置 Flex 容器的主轴方向来指定如何在 Flex 容器中放置 Flex 项目。这决定了灵活项目的布局方向。

行
挠性容器的主轴与当前书写模式的内联轴方向相同。主开始和主结束方向分别等同于当前书写模式的内联开始和内联结束方向。
行-反转
与行相同，只是主开始和主结束的方向互换。
列
Flex容器的主轴与当前书写模式的块轴方向相同。主开始和主结束方向分别等同于当前书写模式的块开始和块结束方向。
列-反转
与列相同，只是主开始和主结束方向互换。
注意：反转值不会反转框的顺序：与书写模式和方向[CSS3-WRITING-MODES]一样，它们只改变流动的方向。绘画顺序、讲话顺序和顺序导航顺序不受影响。
...
flex-wrap属性控制flex容器是单行还是多行，以及交叉轴的方向，它决定了新行的堆叠方向。
...
wrap-reverse
与wrap相同。
对于不是wrap-reverse的值，交叉开始方向相当于当前书写模式的inline-start或block-start方向（以交叉轴为准），交叉结束方向是交叉开始的相反方向。当柔性包覆为包覆反转时，交叉开始和交叉结束的方向会互换。
...
flex-flow属性是设置flex-direction和flex-wrap属性的简称，它们共同定义了flex容器的主轴和横轴。

### 9.4.4. order 属性

默认情况下，Flex项目是按照它们在源文档中出现的相同顺序显示和布局的。顺序属性可以用来改变这种顺序。
...
顺序属性通过将其分配给序数组来控制柔性项目在柔性容器中的出现顺序。它接受一个单一的<整数>值，该值指定了该柔性项目属于哪个序数组。

一个灵活的容器按照修改过的文档顺序排列其内容，从编号最低的序数组开始向上排列。具有相同序号组的项目将按照它们在源文档中出现的顺序排列。这也会影响到绘画顺序[CSS21]，就像柔性项目在源文档中被重新排序一样。Flex容器的绝对位置的子项被视为具有顺序。0，以确定其相对于柔性项目的绘画顺序。
